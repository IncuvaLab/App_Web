    ...
    app.controller('TimeEditController', ($scope, $http) => {
        ...
        angular.element(document).ready(() => {
            ...
            $http.get(`/times/${id}`).then((result) => {
                ...
                var f1_start =result.data.form1_start.split('-');
                var f1_end =result.data.form1_end.split('-');
                $scope.time.form1_start = new Date(f1_start[0], f1_start[1] - 1, f1_start[2]);
                $scope.time.form1_end = new Date(f1_end[0], f1_end[1] - 1, f1_end[2]);

                var f2_start =result.data.form2_start.split('-');
                var f2_end =result.data.form2_end.split('-');
                $scope.time.form2_start = new Date(f2_start[0], f2_start[1] - 1, f2_start[2]);
                $scope.time.form2_end = new Date(f2_end[0], f2_end[1] - 1, f2_end[2]);

                var f3_start =result.data.form3_start.split('-');
                var f3_end =result.data.form3_end.split('-');
                $scope.time.form3_start = new Date(f3_start[0], f3_start[1] - 1, f3_start[2]);
                $scope.time.form3_end = new Date(f3_end[0], f3_end[1] - 1, f3_end[2]);

                var f4_start =result.data.form4_start.split('-');
                var f4_end =result.data.form4_end.split('-');
                $scope.time.form4_start = new Date(f4_start[0], f4_start[1] - 1, f4_start[2]);
                $scope.time.form4_end = new Date(f4_end[0], f4_end[1] - 1, f4_end[2]);

                var f5_start =result.data.form5_start.split('-');
                var f5_end =result.data.form5_end.split('-');
                $scope.time.form5_start = new Date(f5_start[0], f5_start[1] - 1, f5_start[2]);
                $scope.time.form5_end = new Date(f5_end[0], f5_end[1] - 1, f5_end[2]);

                var f6_start =result.data.form6_start.split('-');
                var f6_end =result.data.form6_end.split('-');
                $scope.time.form6_start = new Date(f6_start[0], f6_start[1] - 1, f6_start[2]);
                $scope.time.form6_end = new Date(f6_end[0], f6_end[1] - 1, f6_end[2]);

                var f7_start =result.data.form7_start.split('-');
                var f7_end =result.data.form7_end.split('-');
                $scope.time.form7_start = new Date(f7_start[0], f7_start[1] - 1, f7_start[2]);
                $scope.time.form7_end = new Date(f7_end[0], f7_end[1] - 1, f7_end[2]);
